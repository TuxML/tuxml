...
CONTENT_BASE_IMAGE = {
    ...
    'RUN_DEP': "RUN echo deb http://ftp.de.debian.org/debian buster main >> /etc/apt/sources.list\n"
               "RUN apt-get -y update && apt-get -y install python3 python3-dev python3-pip python3-setuptools default-libmysqlclient-dev apt-file apt-utils && apt-get install -y --no-install-recommends --download-only " +
            BASIC_DEP,
